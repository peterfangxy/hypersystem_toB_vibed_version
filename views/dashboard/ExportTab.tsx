...
// Static configuration for icons and colors
const REPORT_CONFIG = {
    members: {
        icon: <Users size={24} />,
        color: 'text-blue-400'
...
const ExportTab = () => {
        ...
        switch(selectedReport) {
                ...
                return (
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('performance.reports.dateJoined')}</label>
            ...
            case 'transactions':
                return (
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('performance.reports.transactionDate')}</label>
                            <div className="flex items-center gap-2">
                                <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className={`w-full ${THEME.input} rounded-xl px-4 py-3 outline-none [color-scheme:dark]`} />
                                <span className="text-gray-500">-</span>
                                <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className={`w-full ${THEME.input} rounded-xl px-4 py-3 outline-none [color-scheme:dark]`} />
                        ...
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('performance.reports.transactionType')}</label>
                            <select 
                                value={transactionType}
                                onChange={e => setTransactionType(e.target.value)}
                                className={`w-full ${THEME.input} rounded-xl px-4 py-3 outline-none`}
                            >
                                <option value="all">{t('performance.reports.allTransactions')}</option>
                                <option value="deposit">{t('performance.reports.depositsOnly')}</option>
                                <option value="withdrawal">{t('performance.reports.withdrawalsOnly')}</option>
                            </select>
                        </div>
                    </div>
                );
            ...
            case 'revenue':
                return (
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">{t('performance.reports.eventDate')}</label>
                            <div className="flex items-center gap-2">
                                <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className={`w-full ${THEME.input} rounded-xl px-4 py-3 outline-none [color-scheme:dark]`} />
                                <span className="text-gray-500">-</span>
                                <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className={`w-full ${THEME.input} rounded-xl px-4 py-3 outline-none [color-scheme:dark]`} />
    ...
    const getReportDetails = (id: ReportType) => {
        return {
            title: t(`performance.reports.${id}.title`),
            description: t(`performance.reports.${id}.desc`),
            ...REPORT_CONFIG[id]
        };
    };
    ...
    return (
        <div className="animate-in fade-in zoom-in-95 duration-500 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {(Object.keys(REPORT_CONFIG) as ReportType[]).map(id => {
                    const report = getReportDetails(id);
                    return (
                        <button 
                            key={id}
                        ...
                        >
                            ...
                            <div className="mt-auto pt-6 flex items-center text-brand-green text-sm font-bold opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-2 group-hover:translate-y-0">
                                {t('common.manage')} <FileText size={16} className="ml-2" />
                            </div>
                        </button>
                    );
                })}
                ...
                title={
                    selectedReport && (
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg bg-[#1A1A1A] border border-[#333] ${REPORT_CONFIG[selectedReport].color}`}>
                                {REPORT_CONFIG[selectedReport].icon}
                            </div>
                            <span>{t(`performance.reports.${selectedReport}.title`)}</span>
                        </div>
                    )
                }
                size="md"
                ...
                <div className="p-6">
                    <div className="mb-6 p-4 bg-[#1A1A1A] border border-[#333] rounded-xl flex items-start gap-3">
                        ...
                        <div className="text-sm text-gray-400">
                            {t('performance.reports.configureFilters')}
                        </div>
                    </div>
                    ...
                    <div className="mt-8 flex gap-3">
                        ...
                        <Button 
                            ...
                            icon={Download}
                        >
                            {t('performance.reports.exportData')}
                        </Button>
                    </div>
                </div>
            </Modal>
